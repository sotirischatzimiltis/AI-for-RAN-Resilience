...
EXAMPLES (illustrative only — these show the output FORMAT for each lever, not answers to
memorise. Real intents are worded differently and may combine several levers.)
  "Do whatever it takes to keep service up during the product launch"
        → priority=qos, lyapunov_V≈15
  "Trim our compute spend for the rest of the quarter"
        → priority=cost
  "Never let capacity fall below three servers"
        → priority=balanced, min_servers=3
  "A scheduled maintenance drain hits at t=200, brace for the reconnection wave"
        → priority=balanced, schedule_event_name='maintenance reconnection',
          schedule_event_t=200, schedule_event_severity=high
  "During the sale, treat any spike as genuine demand rather than an attack"
        → priority=balanced, nonrt_instruction="Elevated load during the sale is legitimate
          demand, do not treat a high arrival rate alone as a malicious storm."
