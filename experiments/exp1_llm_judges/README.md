...
## Result — `llm_judges_rho_serial.json`
...
## Reproduce
```bash
python -m scripts.exp_1_llm_judges \
  --prompt prompts/non_rt_agent_system_prompt_v3.md --tag rho_serial --serial \
  --seeds 5 --save --resume --log
```

Per-episode reasoning/utility traces (`reasoning/`) and run logs (`logs/`) are regenerable and
git-ignored; only this README and `llm_judges_rho_serial.json` are tracked.
